...
text in any document. A HALO has since been confirmed on a bus (see
"Confirmed against hardware" below), but that confirmation didn't come
with a report of which exact pins were used to get there, so this
specific pin assignment is still unconfirmed independently. Promote it
once someone wires it exactly as described here and reports back.
...
| Local Modbus control (mode/dispatch writes, not just telemetry reads) works in day-to-day use | Hybrid + HALO | Beyond the specific fixes above, multiple users run Modbus mode control in production without reporting further issues -- including at least one running coordinated writes across two HALO towers for their own automation. This is evidence the feature works as a whole, not confirmation of any individual register's exact semantics; the "Still unverified" table below still lists plenty of specifics (sign conventions, dispatch-mode interactions, most of the wider register map) that this doesn't settle. |
