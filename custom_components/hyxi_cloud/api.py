        ...
        try:
            async with self.session.post(
                ...
                if token_val:
                    ...
                    _LOGGER.debug("HYXi API returned raw token expiration: %s seconds", raw_expires_in)
                    ...
                    refresh_time_str = datetime.fromtimestamp(self.token_expires_at).strftime('%Y-%m-%d %H:%M:%S')
                    _LOGGER.debug(
